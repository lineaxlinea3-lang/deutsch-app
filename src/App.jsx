...
function normalizeDe(str) {
  return str.toLowerCase().replace(/[!?.,;:¡¿]/g,"").replace(/ß/g,"ss").replace(/ä/g,"a").replace(/ö/g,"o").replace(/ü/g,"u").trim();
}
function scorePronunciation(expected, heard) {
  ...
  let matched=0; expW.forEach(w=>{ if(gotW.includes(w)) matched++; });
  let charScore=0; const minLen=Math.min(exp.length,got.length);
...
function useSpeechRecognition() {
  ...
  useEffect(() => {
    ...
    const r = new SR(); r.lang="de-DE"; r.continuous=false; r.interimResults=false; r.maxAlternatives=3;
...
Reglas: artículo en sustantivos, phonetic silábica mayúsculas, tip máx 8 palabras. Verbos: primera persona.`;
  ...
  return JSON.parse(text.replace(/```json|```/g,"").trim()).cards||[];
}
...
function PronunciationPanel({card, theme, onSpeak, speaking}) {
  ...
  useEffect(()=>{ if(transcript&&card){setResult({...scorePronunciation(card.front,transcript),heard:transcript});setAttempts(a=>a+1);} },[transcript]);

  const glassStyle = { background:"rgba(255,255,255,0.04)", backdropFilter:"blur(20px)", WebkitBackdropFilter:"blur(20px)", border:`1px solid ${theme.border}` };

  if(!supported) return <div style={{...glassStyle,borderRadius:16,padding:"14px",fontSize:12,color:"rgba(255,255,255,0.3)",textAlign:"center",marginBottom:12}}>🎤 Usa Chrome o Edge</div>;

  return (
    <div style={{...glassStyle, borderRadius:20, overflow:"hidden", marginBottom:14, width:"100%", maxWidth:400, animation:"slideUp 0.3s ease"}}>
      <div style={{padding:"11px 18px 9px",borderBottom:"1px solid rgba(255,255,255,0.05)",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
      ...
      <div style={{padding:"14px 18px"}}>
        ...
        {/* Step 2 */}
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:listening?12:0}}>
        ...
        {result&&!listening&&(
          <div style={{background:"rgba(0,0,0,0.3)",borderRadius:14,padding:"14px",border:`1px solid ${result.color}25`,marginTop:10,animation:"slideUp 0.3s ease"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
              <span style={{fontSize:22}}>{result.emoji}</span>
              <span style={{fontSize:26,fontWeight:800,color:result.color}}>{result.score}%</span>
            </div>
            <div style={{height:4,background:"rgba(255,255,255,0.05)",borderRadius:2,marginBottom:8,overflow:"hidden"}}>
              <div style={{height:"100%",borderRadius:2,background:result.color,width:`${result.score}%`,transition:"width 1s cubic-bezier(.17,.67,.25,1.2)",boxShadow:`0 0 8px ${result.color}`}}/>
            </div>
            <div style={{fontSize:13,color:result.color,fontWeight:600,marginBottom:10}}>{result.label}</div>
...
export default function DeutschAI() {
  ...
  useEffect(()=>{
    if((deck.length<6 || (deck.length>0&&index>=deck.length-5)) && !generatingRef.current)
      loadMoreCards();
  },[deck.length,index]);
  ...
  function markKnown() { if(!card) return; setKnown(s=>new Set([...s,index])); setUnknown(s=>{const n=new Set(s);n.delete(index);return n;}); go("next"); }
  function markUnknown() { if(!card) return; setUnknown(s=>new Set([...s,index])); setKnown(s=>{const n=new Set(s);n.delete(index);return n;}); go("next"); }
  ...
  return (
    <div style={{
      ...
      {/* Background orbs */}
      <div style={{position:"fixed",left:"10%",top:"15%",width:320,height:320,borderRadius:"50%",background:theme.glow,filter:"blur(80px)",pointerEvents:"none",animation:"orbFloat 7s ease-in-out infinite alternate",opacity:0.6}}/>
      <div style={{position:"fixed",right:"5%",top:"55%",width:250,height:250,borderRadius:"50%",background:level==="A1"?"rgba(29,185,84,0.12)":"rgba(30,144,255,0.12)",filter:"blur(70px)",pointerEvents:"none",animation:"orbFloat 9s ease-in-out infinite alternate-reverse",opacity:0.5}}/>
      <div style={{position:"fixed",left:"40%",bottom:"10%",width:200,height:200,borderRadius:"50%",background:"rgba(120,40,200,0.1)",filter:"blur(60px)",pointerEvents:"none",animation:"orbFloat 11s ease-in-out infinite alternate"}}/>

      {/* Header */}
      <div style={{textAlign:"center",marginBottom:24,position:"relative",animation:"fadeDown 0.5s ease"}}>
        ...
        {totalGenerated>0&&<div style={{fontSize:11,color:theme.accent,marginTop:4,opacity:0.8}}>✦ {deck.length} tarjetas</div>}
      </div>

      {/* Level tabs */}
      <div style={{display:"flex",gap:8,marginBottom:14,animation:"fadeDown 0.5s ease 0.1s both"}}>
        {LEVELS.map(l=>{
          const t=THEME[l]; const active=level===l;
          return (
            <button key={l} onClick={()=>setLevel(l)} style={{
              padding:"9px 26px", borderRadius:30,
              background:active?`linear-gradient(135deg,${t.accent}20,${t.accent}0a)`:"rgba(255,255,255,0.03)",
              border:`1px solid ${active?t.accent:"rgba(255,255,255,0.07)"}`,
              color:active?t.accent:"rgba(255,255,255,0.3)",
              fontSize:13, fontWeight:700, cursor:"pointer",
              boxShadow:active?`0 0 22px ${t.glow}`:"none",
              transition:"all 0.3s ease",
              transform:active?"scale(1.05)":"scale(1)",
            }}>
              {l} <span style={{fontSize:10,opacity:0.6}}>{t.label}</span>
            </button>
          );
        })}
      </div>
      ...
      {/* Category tabs */}
      <div style={{display:"flex",gap:6,marginBottom:22,flexWrap:"wrap",justifyContent:"center",animation:"fadeDown 0.5s ease 0.15s both"}}>
        {CATEGORIES.map(c=>{
          const active=category===c;
          return (
            <button key={c} onClick={()=>setCategory(c)} style={{
              padding:"7px 15px", borderRadius:22,
              background:active?theme.glass:"transparent",
              border:`1px solid ${active?theme.accent:"rgba(255,255,255,0.07)"}`,
      ...
      {/* Card */}
      {!card ? (
        <div style={{width:"100%",maxWidth:400,minHeight:220,borderRadius:24,...glass,display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:12}}>
        ...
        <div
          ...
          style={{
            ...
            opacity:animDir?0:1,
            transition:"transform 0.2s ease,opacity 0.2s ease,border-color 0.4s,box-shadow 0.4s",
            marginBottom:14,
          }}
        >
          ...
          {!flipped ? (
            <div style={{textAlign:"center"}}>
              <div style={{fontSize:36,fontWeight:800,letterSpacing:-1.5,marginBottom:16,lineHeight:1.1,textShadow:`0 0 40px ${theme.glow}`}}>
                {card.front}
              </div>
              <button onClick={(e)=>{e.stopPropagation();speakGerman(card.front,()=>setSpeaking(true),()=>setSpeaking(false));}} style={{
                ...
                border:`1px solid ${speaking?theme.accent:"rgba(255,255,255,0.1)"}`,
                borderRadius:28, padding:"8px 20px",
                display:"inline-flex", alignItems:"center", gap:8, cursor:"pointer",
                boxShadow:speaking?`0 0 20px ${theme.glow}`:"none",
                transition:"all 0.25s ease",
              }}>
                <span style={{fontSize:16,animation:speaking?"speakPulse 0.6s ease infinite alternate":"none"}}>🔊</span>
                <span style={{fontSize:12,color:speaking?theme.accent:"rgba(255,255,255,0.3)",fontStyle:"italic"}}>
            ...
            <div style={{textAlign:"center"}}>
              <div style={{fontSize:30,fontWeight:700,color:theme.accent,marginBottom:12,lineHeight:1.2,textShadow:`0 0 30px ${theme.glow}`}}>
                {card.back}
              </div>
              <button onClick={(e)=>{e.stopPropagation();speakGerman(card.front,()=>setSpeaking(true),()=>setSpeaking(false));}} style={{
                background:speaking?theme.glass:"rgba(255,255,255,0.04)",
                border:`1px solid ${speaking?theme.accent:"rgba(255,255,255,0.08)"}`,
                borderRadius:24, padding:"6px 16px",
                display:"inline-flex", alignItems:"center", gap:6, cursor:"pointer",
                marginBottom:showTip?14:0, transition:"all 0.2s",
              }}>
                <span style={{fontSize:14,animation:speaking?"speakPulse 0.6s ease infinite alternate":"none"}}>🔊</span>
                <span style={{fontSize:11,color:speaking?theme.accent:"rgba(255,255,255,0.3)",fontStyle:"italic"}}>
                  {speaking?"reproduciendo...":card.phonetic}
                </span>
              </button>
              {showTip&&(
      ...
      <div style={{display:"flex",gap:8,marginBottom:14,flexWrap:"wrap",justifyContent:"center"}}>
        ...
        {card&&(
          <button onClick={()=>setShowPronunciation(s=>!s)} style={{
            background:showPronunciation?theme.glass:"transparent",
            border:`1px solid ${showPronunciation?theme.accent:"rgba(255,255,255,0.08)"}`,
            color:showPronunciation?theme.accent:"rgba(255,255,255,0.28)",
            padding:"6px 16px", borderRadius:20, fontSize:11, cursor:"pointer",
            boxShadow:showPronunciation?`0 0 12px ${theme.glow}`:"none",
            transition:"all 0.2s",
          }}>
            🎤 {showPronunciation?"Cerrar":"Pronunciación"}
          </button>
        )}
      </div>
      ...
      <div style={{display:"flex",gap:10,marginBottom:22,alignItems:"center"}}>
        <button onClick={()=>go("prev")} disabled={index===0} style={{
          ...
          color:index===0?"rgba(255,255,255,0.1)":"rgba(255,255,255,0.5)",
          fontSize:18, cursor:index===0?"not-allowed":"pointer",
      ...
      {generating&&(
        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:16,fontSize:12,color:theme.accent,animation:"fadeDown 0.3s ease"}}>
      ...
      <button onClick={loadMoreCards} disabled={generating} style={{
        ...
        color:generating?"rgba(255,255,255,0.1)":"rgba(255,255,255,0.18)",
      ...
      <div style={{marginTop:28,fontSize:10,color:"rgba(255,255,255,0.06)",letterSpacing:4,textTransform:"uppercase"}}>
      ...
      <style>{`
        * { box-sizing:border-box; }
        ...
        @keyframes orbFloat { from{transform:translate(-50%,-50%) scale(1)} to{transform:translate(-50%,-50%) scale(1.18) translateY(-18px)} }
